...
export function prepareInvocation(request, promptFile) {
...
export async function runGrok(request, spawn = spawnGrok) {
    ...
    try {
        ...
        const paths = extractMediaPaths(`${spawned.stdout}\n${text}`, request.kind === 'text' ? 'any' : request.kind);
...
export function grokAsk(request, spawn) {
    return runGrok({ ...request, kind: 'text' }, spawn);
}
export function grokImagineImage(request, spawn) {
    return runGrok({ ...request, kind: 'image' }, spawn);
}
export function grokImagineVideo(request, spawn) {
